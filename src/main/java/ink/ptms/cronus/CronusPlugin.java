 ...
 */
public abstract class CronusPlugin extends JavaPlugin {
    ...
    protected static CronusPlugin plugin;
...
//    }
    ...
    private static Class<?> getMainClass() {
        ...
        try {
            ZipFile zipFile = new ZipFile(toFile(CronusPlugin.class.getProtectionDomain().getCodeSource().getLocation().openStream(), file));
    ...
    private static void init() {
        ...
        if (!libFile.exists()) {
            // 本地资源检测
            InputStream resourceAsStream = CronusPlugin.class.getClassLoader().getResourceAsStream("TabooLib.jar");
    ...
    public static CronusPlugin getPlugin() {
        return plugin;
    }
